...
$d_fecha=date('Y-m-d');
	...
	if (isset($_POST['busca']) and !empty($_POST['busnom']))
	{
		...
		$_pagi_sql = "SELECT * FROM tem_sicore_saf
		              where saf='$nrosaf'
	...
	
	{
	 ...
	 if ($_GET['_pagi_pg'] >=1)
	   ...
	   $sql = "delete from `tem_sicore_saf`  where saf='$nrosaf' " ;
    	...
    	$sql = "insert into tem_sicore_saf SELECT p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit, s.monto AS retnecion,s.id,s.numero,r.codigo,p.orden_pago,p.saf
	 ...
	 $sql = "insert into tem_sicore_saf SELECT p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit, s.monto AS retnecion,s.id,s.numero,r.codigo,p.orden_pago,p.saf
			...
			$_pagi_sql = "SELECT * FROM tem_sicore_saf where saf='$nrosaf' order by numero DESC,fecha_pago	";		  
...
<h4>CONSTANCIA DE RETENCION SI.CO.RE - Sistema de Control
de Retenciones </h4><br />
