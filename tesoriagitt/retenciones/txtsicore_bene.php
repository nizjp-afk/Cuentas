	   ...
	   elseif ($j==1)
	     {
			...
			$denominacion=substr($datos[mysql_field_name($res_ft,$j)],0,20); 
			echo str_pad($denominacion,20,' ');
			 
		 }
		elseif ($j==2)
	     {
			 $domicilio=substr($datos[mysql_field_name($res_ft,$j)],0,20); 
			echo str_pad($domicilio,20,' ');
			 
		 } 
		 elseif ($j==3)
	     {
			
			 $loclidad=substr($datos[mysql_field_name($res_ft,$j)],0,20); 
			echo str_pad($loclidad,20,' ');
			 
		 } 
		elseif ($j==4)
	     {
			echo str_pad($datos[mysql_field_name($res_ft,$j)],2,' ');
