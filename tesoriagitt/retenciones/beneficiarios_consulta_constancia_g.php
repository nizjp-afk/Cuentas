...
$d_fecha=date('Y-m-d');
	...
	if (isset($_POST['busca']) and !empty($_POST['busnom']))
	{
		...
		$_pagi_sql = "SELECT * FROM tem_sicore
		              where cuit='$usuario'
	...
	
	{
	 ...
	 if ($_GET['_pagi_pg'] >=1)
	   ...
	   $sql = "delete from `tem_sicore`  where cuit='$usuario' " ;
    	...
    	$sql = "insert into tem_sicore SELECT p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit, s.monto AS retnecion,s.id,s.numero,r.codigo,p.orden_pago
	 ...
	 $sql = "insert into tem_sicore SELECT p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit, s.monto AS retnecion,s.id,s.numero,r.codigo,p.orden_pago
			...
			$_pagi_sql = "SELECT * FROM tem_sicore where cuit='$usuario' order BY fecha_pago desc	";		  
