   ...
   if($id=='')
     {
		 ...
		 $ssql = "SELECT *  FROM op_pendientes where Ejercicio > 2009 order by Numero_op ";
...
?>	 	
<form action="indextesoreria.php?sec=retenciones/carga_orden&apli=cr&per=R" method="post" >	
...
if ((isset($_POST['aceptar'])) or ($band=='M'))
   {
	        ...
	        $f_orden =  mysql_fetch_array($r_ord);
			 ...
			 if($est=='B'){$valor='BAJA';}
			 if($est=='R'){$valor='BLOQUEADA';}
			  if($est=='I'){$valor='IMPUESTO';}
			    if($est=='N'){$valor='PENDIENTE';}
...
<form action="indextesoreria.php?sec=retenciones/generar_liquidacion&apli=cr&per=R" method="post" >	
...
<tr> <td colspan="2"> <?php if($est=='B' or $est=='N' )
{
?>
...
<?php if (!($band=='M'))
				    {
?>						
<form action="indextesoreria.php?sec=retenciones/carga_orden&apli=cr&per=R&band=M" method="post" >
<input type="hidden" name="orden" value="<?php echo $orden; ?>" /> 
<input type="hidden" name="id_b" value="<?php echo $id_b; ?>" />
