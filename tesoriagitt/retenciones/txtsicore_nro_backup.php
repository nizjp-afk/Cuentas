...
header("Pragma: no-cache");
 ...
 echo $sql="SELECT s.fecha_io AS fecha_inicio, s.orden, p.total AS monto, r.codigo, s.neto AS base_imponible, p.fecha AS fecha_pago, s.monto AS retnecion, p.cuit, s.numero
...
$sql="SELECT s.fecha_io AS fecha_inicio, s.orden, p.total AS monto, r.codigo, s.neto AS base_imponible, p.fecha AS fecha_pago, s.monto AS retnecion, p.cuit, s.numero
FROM orden_pago_fp AS p, sicore AS s, anexorg830 AS r, beneficiarios_aprobados AS b
WHERE p.orden_pago = s.orden
AND p.ejercicio = s.ejercicio
AND p.saf = s.saf
AND b.sociedad_tipo != '16'
	   ...
	   elseif ($j==1)
		...
		elseif ($j==6)
		  ...
		  elseif ($j==7)
	     {
			...
			$cuit=$datos[mysql_field_name($res_ft,$j)];
			echo str_pad($var1,12,' ',STR_PAD_LEFT).str_pad($cuit,20,' ',STR_PAD_RIGHT).str_pad($var2,15,'0',STR_PAD_LEFT);
		  ...
		  elseif ($j==8)
	     {
			//$var1='80';
			//$var2='00000000000000';
			$comp=explode('-',$datos[mysql_field_name($res_ft,$j)]);
			 $var3=$comp[0].''.$comp[1].''.$comp[2];
			//$cuit=$datos[mysql_field_name($res_ft,$j)];
			echo $var3;
