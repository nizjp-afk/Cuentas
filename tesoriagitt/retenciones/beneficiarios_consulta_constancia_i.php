...
$d_fecha=date('Y-m-d');
	...
	if (isset($_POST['busca']) and !empty($_POST['busnom']))
	{
		...
		$_pagi_sql = "SELECT * FROM tem_sicore_ib 
		              where cuit='$usuario'
	    			AND (numero like '%$nom%' or orden='$nom') 
	...
	
	{
	 ...
	 if ($_GET['_pagi_pg'] >=1)
	  ...
	  $sql = "delete from `tem_sicore_ib`  where cuit='$usuario' " ;
	 ...
	 $sql = "insert into tem_sicore_ib SELECT  p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit,s.monto AS retnecion,s.id,s.numero,p.orden_pago
	   ...
	   $sql = "insert into tem_sicore_ib SELECT  p.fecha AS fecha_pago, b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit,s.monto AS retnecion,s.id,s.numero,p.orden_pago
    	...
    	$_pagi_sql = "SELECT * FROM tem_sicore_ib where cuit='$usuario' order by numero DESC,fecha_pago";
...
?>		
		 ...
		 <td colspan="4">&nbsp;<a href="retenciones/imprimir_ib.php?id=<?php echo $f_persona['numero'];?>&firstinput=<?php echo $fechahoy; ?>&secondinput=<?php echo $fechaant; ?>" target="_blank"><img width="22" height="22" src="img/impresion1.jpg" border="0"/></a>&nbsp;</td>
