...
$d_fecha=date('Y-m-d');
	...
	$nom = trim($_POST['busnom']);
		...
		$_pagi_sql = "SELECT * FROM tem_sicore_ss_saf
		              where saf='$nrosaf'
	 ...
	 if ($_GET['_pagi_pg'] >=1)
      ...
      /*   if($d >=15)
		...
		*/
		 ...
		 $sql = "delete from `tem_sicore_ss_saf`  where saf='$nrosaf' " ;
    	...
    	$sql = "insert into tem_sicore_ss_saf SELECT p.fecha AS fecha_pago,b.apellido,b.nombre,b.nombre_f,razon_social, p.cuit,s.monto AS retnecion,s.id,s.numero_ss,r.codigo,p.orden_pago,p.saf
			...
			$_pagi_sql = "SELECT * FROM tem_sicore_ss_saf where saf='$nrosaf' order by numero_ss desc,fecha_pago	";		  
