...
$sql="SELECT r.codigo,p.cuit,p.fecha,s.monto AS retnecion,s.id,s.numero_ss
...
while($datos = mysql_fetch_assoc($res_ft))
	...
	for($j=0; $j<$columnas; $j++)
	{
	   ...
	   elseif ($j==1)
		...
		elseif ($j==3)
	     {
		   echo str_pad($datos[mysql_field_name($res_ft,$j)],9,' ',STR_PAD_LEFT);
...
while($datos1 = mysql_fetch_assoc($res))
	...
	for($j=0; $j<$columnas; $j++)
	{
	   ...
	   elseif ($j==1)
		...
		elseif ($j==3)
	     {
		   echo str_pad($datos1[mysql_field_name($res,$j)],9,' ',STR_PAD_LEFT);
