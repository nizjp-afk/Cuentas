...
FROM orden_pago AS p, sicore_ss AS s, anexoss AS r,dd_retenciones as d
	   ...
	   if ($j==0)
	      {
			...
			echo str_pad($datos[mysql_field_name($res_ft,$j)],3,' ',STR_PAD_LEFT);
		...
		elseif ($j==2)
	     {
			 ...
			 $monto=$datos[mysql_field_name($res_ft,3)];
			 $fecha_p=explode('-',$datos[mysql_field_name($res_ft,$j)]);
			 $fecha_po=$fecha_p[2].'/'.$fecha_p[1].'/'.$fecha_p[0];
			 echo str_pad($monto,9,' ',STR_PAD_LEFT).$fecha_po;
			 
		 } 
		elseif ($j==3)
	     {
			 $mon='0.00';
		   echo str_pad($mon,9,' ',STR_PAD_LEFT);
			   
		 }  
		 elseif ($j==4)
	     {
		   $id=$datos[mysql_field_name($res_ft,$j)];
			   
		 }  
		elseif ($j==5)
	     {
			 $numero = $datos[mysql_field_name($res_ft,$j)];
			 echo str_pad($numero,8,' ',STR_PAD_LEFT);
			/* if($numero=='0')
			   {
		         echo str_pad($valor,9,' ',STR_PAD_LEFT);
	...
	for($j=0; $j<$columnas; $j++)
	{
	   if ($j==0)
	      {
			...
			echo str_pad($datos1[mysql_field_name($res,$j)],3,' ',STR_PAD_LEFT);
	   ...
	   elseif ($j==1)
		...
		elseif ($j==2)
	     {
			 ...
			 $monto=$datos1[mysql_field_name($res_ft,3)];
			 $fecha_p=explode('-',$datos1[mysql_field_name($res_ft,$j)]);
			 $fecha_po=$fecha_p[2].'/'.$fecha_p[1].'/'.$fecha_p[0];
			 echo str_pad($monto,9,' ',STR_PAD_LEFT).$fecha_po;
		...
		elseif ($j==3)
		 ...
		 elseif ($j==4)
	     {
		   $id=$datos1[mysql_field_name($res,$j)];
