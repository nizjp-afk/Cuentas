...
$aplicacion = $_GET['apli'];
   ...
   '<br>';
	...
	if(empty ($fecha_io) and ($estado=='P') )
...
if ($estado=='P') 
    {
		...
		//$observacion='Orden Visada ';
		 ...
		 $sql="SELECT * FROM `orden_pago` where CLAVE='$id' ";

       if (!($r_hist = mysql_query($sql, $conexion_mysql)))
         {
            //.....................................................................
		...
		$fila =  mysql_fetch_array($r_hist);
		  ...
		  if($est_op=='B'){ $accion='DES_BLO';}else { $accion='PENDIENTE';}
		     
		 
	   
	    $sql="INSERT INTO `histo_orden_pago` ( `CLAVE` , `EJERCICIO` , `SAF` , `FECHA_OP` , `FORMULARIO` , `NUMERO` , `CUIT` , `BENEFICIARIO` , `CONCEPTO` , `FECHA_ACCION` , `HORA_ACCION` , `ACCION` , `Observacion`,USUARIO )
VALUES ('$clave' , '$ejercicio','$saf','$fecha','$formulario','$numero','$cuit','$benef','$concepto',
'$fecha_r','$hora','$accion','$dato_h','$usuario');";

       if (!($r_his_fp = mysql_query($sql, $conexion_mysql)))
         {
            //.....................................................................
            // INFORMA  DE ERROR PRODUCIDO//
...
$sql="update orden_pago set ESTADO='$estado' 
			            where CLAVE='$id'
						";
...
$sql="update orden_pago_temp set ESTADO='$estado' 
			            where CLAVE='$id'
						";
	...
	$sql="update orden_pago_temp_saf set ESTADO='$estado' 
			            where CLAVE='$id'
						";
 ...
 if($estado=='B' or $estado=='A')
   {
       ...
       if (!($r_hist = mysql_query($sql, $conexion_mysql)))
		...
		$fila =  mysql_fetch_array($r_hist);
		  ...
		  if($estado=='B'){ $accion='BLOQ_OP';}
		  
		  if($estado=='A'){ $accion='ANU_OP';}
	   
	    $sql="INSERT INTO `histo_orden_pago` ( `CLAVE` , `EJERCICIO` , `SAF` , `FECHA_OP` , `FORMULARIO` , `NUMERO` , `CUIT` , `BENEFICIARIO` , `CONCEPTO` , `FECHA_ACCION` , `HORA_ACCION` , `ACCION` , `Observacion`,USUARIO )
VALUES ('$clave' , '$ejercicio','$saf','$fecha','$formulario','$numero','$cuit','$benef','$concepto',
'$fecha_r','$hora','$accion','$observacion','$usuario');";
	...
	$numero=$f_num['numero'];     
	 ...
	 $sql="INSERT INTO nota_retencion (orden,saf,numero,observacion,fecha,accion)
        VALUES('$orden','$saf','$numero','$observacion','$fecha_r','$estado')";
...
if ($cant >0)
  ...
  {
   $dato=$fecha_r.': Orden Visada -'.$observacion;
...
///////////////////antecedente
 ...
 if ($estado=='P') 
 {
 if (!(($cuit_bene=='33709641099' ) or ($cuit_bene== '30711009791') 
 or ($cuit_bene== '30711315744') or ($cuit_bene=='30709842931' )
or ($cuit_bene== '30711051801') or ($cuit_bene== '30710994907') ))
       ...
       if (!($r_ef = mysql_query($sql, $conexion_mysql)))
         {
          ...
          $accion='Cambio Estado OP'.': '.$orden.' ,'.'estado'.' = '.$estado;
...
<?php
  if($estado=='P')
    {
?>		
<meta http-equiv='refresh' content='3;url=indextesoreria.php?sec=retenciones/carga_orden_fp&apli=cr&per=R'> 
   ...
   <?php
	...
	{
?>
<meta  http-equiv='refresh' content='5;url=indextesoreria.php?sec=retenciones/carga_orden_fp&apli=cr&per=R'> 
...
<code>O haga click <a href='indextesoreria.php?sec=retenciones/carga_orden_fp&apli=cr&per=R'>aqu&iacute;</a> para regresar.</code>
	  <br><br>
      Gracias
	  </font>
	  </p>
