...
//error_reporting ( E_ERROR );
 ...
 //echo $total=preg_replace('/^(\d+)\.0+$/', '$1', $valor_p);echo '<BR>';
 //$total = rtrim($total, "0");
		...
		if($entidad=='999' or $entidad=='309')
		 ...
		 $sql = "select * from beneficiarios_aprobados where codigo_esidif='$beneficiario'";
			...
			if ($fuente =='1.1')
			    {	      
				echo	 $ssql = "INSERT INTO `orden_pago` 
		...
		if($entidad=='999')
		   {
			 ...
			 if($cuenta=='991014/00'){ $escritural='33'; }else { $escritural='197'; }
 ...
 include('dgti-mysql-var_dbtgp.php');
	...
	echo $ssql = "UPDATE `orden_pago` SET ESTADO='C',importe_a_pagar=0  WHERE `clave` = '$clave' and trim(importe_a_pagar)=trim('$total')";
	...
	echo $ssql = "INSERT INTO `orden_pago_fp` 
		        (`cuit` ,`saf`,`fuente`,`clase`,`orden_pago`,`fecha`,`concepto`,`importe`,`retencion`,
