...
if (($_GET['busnom']!='') or (isset($_POST['busca'])) or (isset($_GET['_pagi_pg'] )))
     ...
     if ((isset($_POST['busca']) and !empty($_POST['busnom'])))
		{
				...
				$_pagi_sql = "SELECT CASE estado
								...
								WHEN 'I'
								THEN 'I'
								END AS estado, op_pendientes . * FROM op_pendientes where (Saf='$nom' or Numero_OP='$nom') AND
								  Ejercicio='$fecha_cons' ORDER BY `estado` DESC,Numero_OP desc";
	 ...
	 elseif ($_GET['_pagi_pg'] >=1 and !empty($_GET['nom']) and (!(isset($_POST['busca'])))) 
        {
		 ...
		 $_pagi_sql = "SELECT CASE estado
						...
						WHEN 'I'
						THEN 'I'
						END AS estado, op_pendientes . * FROM op_pendientes where (Saf='$nom' or Numero_OP='$nom') AND Ejercicio='$fecha_cons' 
						ORDER BY `estado` DESC,Numero_OP desc";
		 ...
		 $nom = $_GET['busnom'];
		  $_pagi_sql =  "SELECT CASE estado
		   ...
		   $_pagi_sql = "SELECT CASE estado
							...
							WHEN 'I'
							THEN 'I'
							END AS estado, op_pendientes . * FROM op_pendientes where Ejercicio='$fecha_cons' ORDER BY `estado` 
							DESC,Numero_OP desc"; 
									   }
	   }
  }
 else
  {
	 ...
	 $_pagi_sql = "SELECT CASE estado
					...
					WHEN 'I'
					THEN 'I'
					END AS estado, op_pendientes . * FROM op_pendientes where Ejercicio='$fecha_cons' ORDER BY `estado` DESC,Numero_OP desc "; 
  }
