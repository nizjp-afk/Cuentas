...
//conexion
 ...
 $ssql = "SELECT * FROM `clasificador` where codigo in('P','S') order by codigo, concepto asc ";
...
<form name="" action="indextesoreria.php?sec=clasificador/clasificador&apli=tgpa&per=T" method="post">  
...
<table width="80%"  align="left" border="1" cellpadding="0" cellspacing="0" bgcolor="#DBE3E6" bordercolor="#EAEAEA" >
...
-->     <tr>
     <td height="45" colspan="8" align="left" > 
          <font color="#333333" size="2" face="Verdana, Arial, Helvetica, sans-serif">CLASIFICADOR: </font>
...
?>
<OPTION value="<?php echo $f_saf['id'] ?>"> <?php echo $f_saf['concepto']?></OPTION>
      ...
      while ($f_orden=mysqli_fetch_array($r_clasi))
	  {
		...
		WHERE cuitl='$cuit'";
		  ...
		  $f_cui =  mysqli_fetch_array($res_cui);
		   ...
		   $ssql = "SELECT *  FROM clasificacion_orden where ejercicio='$ejercicio' and orden='$orden_pago' and saf='$saf' and fecha='$fecha' ";
...
?>	
	    ...
	    <tr bgcolor="#F3F3F3"    class="letra"> 
          ...
          <td><?php echo substr($benef,0,20) ;?></td>
          <td><strong><?php echo substr($f_orden['concepto'],0,50);?></strong></td>
