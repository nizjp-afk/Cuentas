...
$nuevafecha = date ( 'Y-m-d' , $nuevafecha );
 ...
 if($ej_a=='EA')
   { 
	 ...
	 if ($sub=='N')
	 {
	   $_pagi_sql = "SELECT op_pendientes. * FROM op_pendientes,beneficiarios_aprobados 
				              where (Numero_OP='$nom' or cuit ='$nom' or concepto like '%$nom%' )
							  and Ejercicio <'$y'
		 ...
		 if ($sub=='N')
	 {
		...
		$_pagi_sql_sp = "SELECT SUM(Saldos) as Saldos,SUM(Imp_orden) as Imp_orden,SUM(Total_Pagado) as Total_Pagado FROM op_pendientes,beneficiarios_aprobados
				              where (Numero_OP='$nom' or cuit ='$nom' or concepto like '%$nom%' )
							  and Ejercicio < '$y'
							  and op_pendientes.estado='N'
							 and beneficiarios_aprobados.cuitl=op_pendientes.cuit
							  and sociedad_tipo='30'
							  and `Fecha_OP` >  '$nuevafecha'
							  and (`Fecha_OP` !='2015-03-30' and `Fecha_OP` !='2015-03-31')
 ...
 $cont=0;
   ...
   {   
      ...
      if ($sub=='N')
	 {
        ...
        $_pagi_sql1 = "SELECT ejercicio, o.saf, orden_pago,fuente, fecha, apellido, nombre, razon_social, concepto, total
 ...
 $aux=0;
   ...
   else
   {
	   ...
	   if ($sub=='N')
	 {
	       ...
	       $_pagi_sql1 = "SELECT ejercicio, o.saf, orden_pago, fecha,fuente, apellido, nombre, razon_social, concepto, total
FROM orden_pago AS o, beneficiarios_aprobados AS b
				              where (orden_pago='$nom' or cuit ='$nom' or concepto like '%$nom%') 
							      and cuitl=cuit
								  and b.sociedad_tipo='30'
