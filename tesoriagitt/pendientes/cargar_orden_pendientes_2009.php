...
if ((isset($_POST['busca']) and !empty($_POST['busnom'])) or ( $_POST['busnom']!=''))
			{
				...
				AND Ejercicio='$fecha_cons' and estado='N'";
	...
	elseif (($_GET['busnom']!='' or $_GET['nom']!='') and (!(isset($_POST['busca']))) )
	  ...
	  else
	   {
		  ...
		  $nom='';
		  $ssql = "SELECT sum(`Saldos`) as saldo,sum(`Imp_orden`) as importe,sum(`Total_Pagado`) as total,count( `Numero_OP` ) as cant 
		  FROM op_pendientes where Ejercicio='$fecha_cons' and estado='N' ";
				 if (!($r_op= mysql_query($ssql, $conexion_mysql)))
				{
				  //.....................................................................
				  // informa del error producido
...
WHEN 'I'
THEN 'I'
END AS estado, op_pendientes . * FROM op_pendientes where (Saf='$nom' or Numero_OP='$nom' or cuit='$nom') AND Ejercicio='$fecha_cons' ORDER BY `estado` DESC,FECHA_OP desc,Numero_OP";
...
$_pagi_propagar=array('sec','nom','apli','per','fecha_cons');
...
<table  width="110%"  border="1" align="center" cellpadding="0" cellspacing="0" bgcolor="#DBE3E6" bordercolor="#EAEAEA" >
     <tr class="fuframe1">
    	<td height="30" colspan="3" >Nro. de Orden
         ...
         <td height="30" colspan="2" ><a title="Listado de Ordenes Pendientes" href="tesoreria/listado_pendientes_ft_cons.php?fecha_cons=<?php echo $fecha_cons; ?>&saf=<?php echo $nom; ?>" target="_blank"  ><img src="img/print_odp.jpg" width="35" align="" height="24" border="0"/></a> 
          <a title="Imprimir Consulta" href="pendientes/listado_ordenes_pendientes_xls.php?consul=<?php echo $fecha_cons ?>&nom=<?php echo $nom; ?>&ban=<?php echo $b_eje; ?>" target="_blank"  ><img src="img/xls.jpg"  width="35" align="" height="24" border="0"/></a>   
		</td>     
