...
if ((isset($_POST['busca']) and !empty($_POST['busnom'])) or ( $_POST['busnom']!=''))
			{
				...
				$ssql = "SELECT SUM( IMPORTE_A_PAGAR ) AS saldo, SUM( IMP_FORM - RETENCION - IMPORTE_A_PAGAR ) AS total, SUM( IMP_FORM - RETENCION ) AS importe, count(NUMERO) AS cant
						...
						AND orden_pago.SAF='$nrosaf' 
						AND e.GRUPO='$sub_nrosaf'
						AND e.ESTADO='A'
						AND ((NUMERO='$op_n' and FORMULARIO='$op_f' AND orden_pago.SAF='$op_s') or CUIT ='$nom' )";
...
$_pagi_propagar=array('sec','nom','apli','per','fecha_cons','sub_saf','fecha_ant');
...
<table  width="110%"  border="1" align="center" cellpadding="0" cellspacing="0" bgcolor="#DBE3E6" bordercolor="#EAEAEA" >
     <tr class="fuframe1">
      ...
      <td height="30" colspan="2" ><a title="Listado de Orden de Pago Pendientes" href="consolidada/listado_ordenes_pendientes_fp_sb_bd.php?consul=<?php echo $fecha_cons; ?>&saf=<?php echo $nrosaf; ?>&sub_saf=<?php echo $sub_nrosaf; ?>" target="_blank"  ><img src="img/print_odp.jpg" width="35" align="" height="24" border="0"/></a>   
		</td>   
   ...
   <?php
      ...
      while ($f_orden=mysql_fetch_array($_pagi_result))
	  {
		   ...
		   $Imp_orden=number_format($f_orden['Imp_orden'],2);
            $Total_Pagado=number_format($f_orden['pagado'],2);
...
<ul>
      ...
      <li><a href="indextesoreria.php?sec=pendientes/index1_saf_fp&apli=s&per=A">Regresar Menu</a></li>
