...
$nuevafecha = date ( 'Y-m-d' , $nuevafecha );
	 ...
	 if($ej_a=='EA')
   { 
       $_pagi_sql = "SELECT `Fecha_OP` AS FECHA, op_pendientes.`Saf` AS SAF, `Ejercicio` , `Numero_OP` AS ORDEN, b.cuitl AS CUIT, CONCAT( b.apellido, ' ', b.nombre, ' ', b.razon_social ) AS DENOMINACION, concepto AS CONCEPTO, `Imp_orden` , `Total_Pagado` , `Saldos`,op_pendientes.estado as ESTADO
FROM op_pendientes, beneficiarios_aprobados AS b
WHERE (
Numero_OP = '$nom'
OR cuit = '$nom'
OR concepto LIKE '%$nom%'
)
AND Ejercicio < '$y'
...
echo "<tr>";
echo "<td align='center' colspan='11' bgcolor='#CCCCCC'>ORDENES PENDIENTES DE PAGO</td>";
echo "</tr>";
echo "<BR>";
echo "<tr>";
...
if($ej_a=='EA')
   {   
        $_pagi_sql1 = "SELECT  fecha AS FECHA, o.saf AS SAF,ejercicio AS EJERCICIO, orden_pago AS ORDEN, b.cuitl AS CUIT,  CONCAT( b.apellido, ' ', b.nombre, ' ', b.razon_social ) AS DENOMINACION, concepto AS CONCEPTO, total AS TOTAL 
...
echo "<table border='1'>";
echo "<tr>";
echo "<td align='center' colspan='8' bgcolor='#CCCCCC'>ORDENES PAGADAS</td>";
echo "</tr>";
echo "<BR>";
