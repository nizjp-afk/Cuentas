	...
	else
	  {
		 ...
		 $ssql = "SELECT *  FROM `saf_escritural` where ID='$saf' ORDER BY `ESCRITURAL` ASC";
		  ...
		  $n_saf=$f_saf['ESCRITURAL'];
		   ...
		   $_pagi_sql = "SELECT `nro_saf`. * , `orden_pago_fp`. * 
		               FROM orden_pago_fp, nro_saf
					   WHERE (orden_pago_fp.clave_escritural='$saf')
					   AND orden_pago_fp.saf = numero
...
$pdf->setY(45);
$pdf->cell(0,0,'LISTADO DE ORDENES DE PAGO DE FONDOS PROPIOS '.$n_saf,0,'B','C',0);
   ...
   while($f_saf= mysql_fetch_array ($r_saf))
     {
		  ...
		  $_pagi_sql = "SELECT `nro_saf`. * , `orden_pago_fp`. * 
		               FROM orden_pago_fp, nro_saf
					   WHERE (orden_pago_fp.clave_escritural='$id_es')
					   AND orden_pago_fp.saf = numero
					   AND fecha >='$fechaant' and fecha <= '$fechahoy'
					   ORDER BY ejercicio,`fecha` DESC,orden_pago ASC";
