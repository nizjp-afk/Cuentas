<?php
   ...
   ////////////////////////////
		 ...
		 $_pagi_sql = "SELECT orden_pago.* , (IMP_FORM - RETENCION - IMPORTE_A_PAGAR
) AS pagado, (
IMP_FORM - RETENCION
) AS Imp_orden, e.ESCRITURAL,e.DENOMINACION,e.ID
FROM orden_pago, escritural AS e
WHERE EJERCICIO = '$fecha_cons'
AND orden_pago.SAF='$saf_t'
AND e.ID = orden_pago.id_escritural
AND e.ESTADO='A'
AND (
FECHA_INGRESO !=  '2015-03-30'
AND FECHA_INGRESO !=  '2015-03-31'
)
AND orden_pago.ESTADO != 'C'
and orden_pago.id_escritural not in ('158','159','160','161','162','163','164','165','166','167','168','169','170','171','172','173','174','175','176','177','178','179','180','181','182','183','184','185','186','187','188','189','190','191','192','193','194','195','197')
ORDER BY orden_pago.ESTADO DESC, ESCRITURAL ASC , FORMULARIO, NUMERO DESC";
...
$pdf->SetFont('Arial','I',8);
$pdf->setY(45);
$pdf->cell(0,0,'LISTADO DE ORDENES DE PAGO PENDIENTES  '.$fecha_cons.' - '.'SAF '.$saf_t,0,'B','C',0);
...
$pdf->Cell(23,7,'Escritural',1,0,'C',1);
...
$est='';
	 ...
	 //  $concepto=substr($f_ordenes['Concepto'],0,15);
		   ...
		   $num=$f_ordenes_p['FORMULARIO']; 
		    $ESCRITURAL=$f_ordenes_p['ESCRITURAL']; 
           ...
           $Total_Pagado=number_format($f_ordenes_p['pagado']);
 ...
 $resultado_resta = restaFechas($fecha_ing,$diaa);
	...
	$pdf->Cell(23,6,$ESCRITURAL,1,0,'C',1);
	$pdf->SetFont('courier',$negra,6);
