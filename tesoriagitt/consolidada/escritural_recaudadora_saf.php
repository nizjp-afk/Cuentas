...
if(($saf=='N') and ($cuenta=='N'))
 ...
 /*while ($f_saf=mysql_fetch_array($r_saf))
	 ...
	 $ssql = "SELECT * FROM escritural AS e,saf_escritural as s where e.saf='$saf_o' and s.SAF=e.saf
...
while ($f_ope=mysql_fetch_array($r_ope))
  ...
  $fecha = '';
    ...
    $ssql = "SELECT * FROM escritural where saf='$saf_o'
...
while($row = mysql_fetch_array($r_op))
{
 ...
 $pdf->SetY($i=$i+7);
	...
	$tot_retenciones=0;
	 ...
	 elseif(($saf!='N') and ($cuenta!='N'))
  {
	 $ssql = "SELECT * FROM escritural AS e,saf_escritural as s where e.num_cuenta='$cuenta' and s.SAF=e.saf
	          AND s.ESTADO='A' and s.saf='$saf'
			  AND e.esc_cuenta = s.escritural
AND cod_proceso =2
	 group by esc_cuenta order by esc_cuenta Asc ";
     if (!($r_ope= mysql_query($ssql, $conexion_mysql)))
			{
