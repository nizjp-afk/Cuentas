	...
	if ((isset($_POST['busca']) and !empty($_POST['busnom'])) or ( $_POST['busnom']!=''))
	 ...
	 if ($_GET['busnom']!='')
	  ...
	  else
	   {
		  $nom='';
		  $ssql = "SELECT sum(`Saldos`) as saldo,sum(`Imp_orden`) as importe,sum(`Total_Pagado`) as total,count( `Numero_OP` ) as cant 
		  FROM op_pendiente_tmp_fp where Ejercicio='$fecha_cons' and nro_ti='$nro' ";
				 if (!($r_op= mysql_query($ssql, $conexion_mysql)))
				{
				  //.....................................................................
				  // informa del error producido
...
?>
<div class="content">
<h2>Desconfirmar Ordenes Pendientes  - Fondos Propios - <?php echo $fecha_cons; ?></h2>
...
?>                  <td  height="28" align="center"><?php echo $f_orden['Ejercicio'];?></td>
          ...
          <td align="center"><a target="_parent" href="indextesoreria.php?sec=consolidada/desconfirmar_pago_fp&apli=h&per=A&id=<?php echo $f_orden['id']; ?>&consul=<?php echo $fecha_cons; ?>"><img src="img/ok.png" border="0" height="16" width="16"/></a></td>
...
<ul>
    ...
    <li><a href="indextesoreria.php?sec=consolidada/modificar_orden_consolidada_fp&consul=<?php echo $fecha_cons; ?>&apli=h&per=A">Modificar Ordenes Autorizadas</a></li>
     ...
     <li><a href="indextesoreria.php?sec=hacienda/index1&consul&apli=h&per=A">Regresar Menu</a></li>
