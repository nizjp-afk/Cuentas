...
IMP_FORM - RETENCION )as Imp_orden FROM orden_pago_temp where (SAF='$nom' or NUMERO='$nom') 
AND EJERCICIO='$fecha_cons'
AND ESTADO !='I'
 AND ESTADO !='C' order by FORMULARIO,SAF,NUMERO";
...
IMP_FORM - RETENCION )as Imp_orden
 FROM orden_pago_temp where EJERCICIO='$fecha_cons' 
AND ESTADO !='I'
 AND ESTADO !='C' 
 order by FORMULARIO,SAF,NUMERO"; 
	       }
...
<table width="110%"  border="1" cellpadding="0" cellspacing="0"  >
   ...
   <?php
    ...
    include('dgti-intranet-mysql_select_db.php');
	   ...
	   if(($estado=='A') and ($inhi=='') and ($estado_op=='P'))
{
	?>	
...
?>        <td  height="30" align="center"><?php echo $f_orden['EJERCICIO'];?></td>
          ...
          <td  align="center"><?php if($estado_op!='A') { ?><a target="_parent" href="indextesoreria.php?sec=consolidada/confirmar_pago_fp&apli=h&consul=<?php echo $fecha_cons; ?>&saf=<?php echo $saf; ?>&per=A&id=<?php echo $f_orden['CLAVE']; ?>&nomb=<?php echo $nom; ?>"><img src="img/ok.png" border="0" height="16" width="16"/></a><?php }?></td>
