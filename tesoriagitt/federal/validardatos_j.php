  ...
  $fecha_alta_web = date("d/m/Y");
   ...
   $ssql = "SELECT * FROM `beneficiarios` where cbu='$banco_cbu'";
     ...
     if (!($r_cbuap= mysql_query($ssql, $conexion_mysql)))
    {
	 ...
	 if ($cant >0 or $cant1>0)
		    {
			$bande=7;
?>			
			  <meta http-equiv='refresh' content='0;url=index.php?sec=beneficiario/error&ban=<?php echo $bande; ?>' />
<?php	
			} 
	}		
...
//nombre de la cuenta denominacion
$patron = "^[[:alpha:]]+([[:space:]]?[[:alpha:]])*$"; 
if (!(ereg($patron, $_POST['banco_denominacion'])) )
	    {
		    $bandera =1; 
			$bandera_bco_denominacion = 1;
		}		
...
<?php
     if ($bandera ==1)
	   {
?>	   
      <form action="indextesoreria.php?sec=federal/j_alta" method="post">
<?php
	   }
	 else
	   {
?>	     
<form action="indextesoreria.php?sec=federal/registro_j" method="post">
<?php 
        }
?>	
...
<input type="hidden" value="<?php echo $banco_denominacion; ?>" name="banco_denominacion"  />
...
<table border="0" align="center">
  <tr>
    <td colspan="2"><h1> Validacion Alta Persona Juridica</h1></td>
  </tr>
  <tr height=10px>
    <td colspan="2"><hr /></td>
  </tr>
  <tr>
    ...
    <input type="hidden" name="cuitl2" value="<?php echo $cuitl;?>" />
    <td><?php 
  ...
  <tr>
    ...
    <td><?php     
              ...
              if ( $f_actividad_s['nombre_actividad']== $f_actividad_p['nombre_actividad'])
				  {
?>
        <img  src="../img/ic_advertencia.gif" border="0" /> <font color="#FF0000"><?php echo 'Selecciono igual actividad que la Actividad Primaria';
							   $bandera=1;?></font>
        <?php				  
				  }
							...
							if (!($bandera_fecha_s==1) and ($bandera==1)) 	  
							   {
							     if (($fechai_s == $fechai_p))
								  ...
								  if ($fechai_s < $fechai_p)
								   {
?>
        <img  src="../img/ic_advertencia.gif" border="0" /> <font color="#FF0000"> <?php echo 'La fecha de la actividad Secundaria no puede ser Inferiror a la fecha de actividad Primaria';?> </font>
        <?php  								   
								   }
							    }
							   }
					          ...
					          if(!($fechaia_s=='-----------'))
							   {
							    if ($bandera_fecha_s == 1)
								  {
?>
        <img  src="../img/ic_advertencia.gif" border="0" /> <font color="#FF0000"> <?php echo $error;?><br />
        <?php echo 'No puede Seleccionar fecha sin seleccionar la actividad' ;?> </font>
        <?php       
						     	  }
							  else 
							      {
?>
        <img  src="../img/ic_advertencia.gif" border="0" /> <font color="#FF0000"> <?php echo 'Selecciono fecha sin seleccionar la actividad 2º';?> </font>
        <?php       
                                  }									
							    }
							  }		
        ...
        <?php       
						   ...
						   if(!($fechac_s == $fechai_p ))
							...
							if($fechac_s > $fechai_p )
							 ...
							 $bandera=1;
?>
        <img  src="../img/ic_advertencia.gif" border="0" /> <font color="#FF0000"> <?php echo 'La fecha de contrato no Puede ser Superior a la Fecha de inicio de actividad primaria';    ?> </font>
        <?php       
                             }
							 }
							}  
...
?>
   ...
   <tr>
			<td class="subtitle">Denominacion de Cta</td>
			<td><?php echo $banco_denominacion;
			      if($banco_denominacion=='')
				      {
?>
                                  <img  src="../img/ic_advertencia.gif" border="0"></img> 
								  <font color="#FF0000"><?php echo $error1;?></font>
